...
async def route_point_to_point(
    ...
    try:
        ...
        # Use trail-capable routing for off-road sports; use ORS for road to allow highways
        ...
        async def _route_with_coords(route_coords: List[List[float]]) -> Dict[str, Any]:
            ...
            if use_brouter:
                # Try GraphHopper first (fast), then BRouter, then ORS as retry
                graphhopper_profile = routing_service.GRAPHOPPER_PROFILES.get(request.sport_type, "mtb")
                brouter_profile = routing_service.BROUTER_PROFILES.get(request.sport_type, "trekking")
                ...
                try:
                    ...
                    gh_start = time.monotonic()
                    gh_task = asyncio.create_task(_try_graphhopper(graphhopper_profile))
                    br_task = asyncio.create_task(_try_brouter(brouter_profile))
                    parsed = await gh_task
                    logger.info(f"GraphHopper success for {len(route_coords)} coords")
                    logger.info(
                        "route_attempt",
                        router="graphhopper",
                        elapsed_ms=int((time.monotonic() - gh_start) * 1000),
                    )
                    if route_direct_distance is not None and parsed.get("distance_meters"):
                        logger.info(
                            "route_metrics",
                            router="graphhopper",
                            direct_m=round(route_direct_distance, 2),
                            route_m=round(parsed["distance_meters"], 2),
                            ratio=round(parsed["distance_meters"] / max(route_direct_distance, 1), 3),
                            points=len(parsed.get("geometry", {}).get("coordinates", [])),
                        )
                    if _is_two_point_long_segment(parsed.get("geometry", {}).get("coordinates", [])):
                        logger.warning("GraphHopper returned 2-point long segment; trying BRouter")
                        br_start = time.monotonic()
                        alt = await br_task
                        logger.info(
                            "route_attempt",
                            router="brouter",
                            elapsed_ms=int((time.monotonic() - br_start) * 1000),
                        )
                        if len(alt.get("geometry", {}).get("coordinates", [])) > 2:
                            parsed = alt
                    if parsed.get("distance_meters") and _is_unreasonable_detour(parsed["distance_meters"]):
                        logger.warning("GraphHopper detour detected; trying BRouter for shorter path")
                        br_start = time.monotonic()
                        alt = await br_task
                        logger.info(
                            "route_attempt",
                        ...
                        if alt.get("distance_meters") and alt["distance_meters"] < parsed["distance_meters"]:
                            parsed = alt
                except Exception as exc:
                    graphhopper_error = exc
                    logger.warning(f"GraphHopper failed, retrying with BRouter: {graphhopper_error}")
                    try:
                        br_start = time.monotonic()
                        if br_task is None:
                            parsed = await _try_brouter(brouter_profile)
                        else:
                            parsed = await br_task
                        logger.info(f"BRouter retry success for {len(route_coords)} coords")
                        logger.info(
                            "route_attempt",
                            router="brouter",
                            elapsed_ms=int((time.monotonic() - br_start) * 1000),
                        ...
                        brouter_error = exc
                        logger.warning(f"BRouter retry failed after GraphHopper failure: {brouter_error}")
                        try:
                            ors_profile = routing_service.ORS_PROFILES.get(request.sport_type, "cycling-regular")
                            ors_start = time.monotonic()
                            parsed = await _try_ors(ors_profile)
                            logger.info(f"ORS retry success for {len(route_coords)} coords")
                            logger.info(
                                "route_attempt",
                                router="ors",
                                elapsed_ms=int((time.monotonic() - ors_start) * 1000),
                            )
                        except Exception as ors_error:
                            logger.warning(f"ORS retry failed after GraphHopper/BRouter failure: {ors_error}")
                            raise RuntimeError(
                                f"GraphHopper failed: {graphhopper_error}; "
                                f"BRouter failed: {brouter_error}; ORS retry failed: {ors_error}"
                            ) from ors_error
                finally:
                    if br_task and parsed and not br_task.done():
                        br_task.cancel()
                        with contextlib.suppress(asyncio.CancelledError):
                            await br_task
            else:
                # Try ORS first, then BRouter as retry
                ors_profile = routing_service.ORS_PROFILES.get(request.sport_type, "cycling-road")
                try:
                    ors_start = time.monotonic()
                    parsed = await _try_ors(ors_profile)
                    logger.info(f"ORS success for {len(route_coords)} coords")
                    logger.info(
                        "route_attempt",
                        router="ors",
                        elapsed_ms=int((time.monotonic() - ors_start) * 1000),
                    ...
                    if _is_two_point_long_segment(parsed.get("geometry", {}).get("coordinates", [])):
                        logger.warning("ORS returned 2-point long segment; trying BRouter")
                        brouter_profile = routing_service.BROUTER_PROFILES.get(request.sport_type, "fastbike")
                        br_start = time.monotonic()
                        alt = await _try_brouter(brouter_profile)
                        logger.info(
                            "route_attempt",
                            router="brouter",
                            elapsed_ms=int((time.monotonic() - br_start) * 1000),
                        ...
                        if len(alt.get("geometry", {}).get("coordinates", [])) > 2:
                            parsed = alt
                    if parsed.get("distance_meters") and _is_unreasonable_detour(parsed["distance_meters"]):
                        logger.warning("ORS detour detected; trying BRouter for shorter path")
                        brouter_profile = routing_service.BROUTER_PROFILES.get(request.sport_type, "fastbike")
                        br_start = time.monotonic()
                        alt = await _try_brouter(brouter_profile)
                        logger.info(
                    ...
                    logger.warning(f"ORS failed, retrying with BRouter: {ors_error}")
                    try:
                        brouter_profile = routing_service.BROUTER_PROFILES.get(request.sport_type, "fastbike")
                        br_start = time.monotonic()
                        parsed = await _try_brouter(brouter_profile)
                        ...
                        logger.warning(f"BRouter retry failed after ORS failure: {brouter_error}")
                        raise RuntimeError(f"ORS failed: {ors_error}; BRouter retry failed: {brouter_error}") from brouter_error
