...
async def _build_no_route_response(request: ChatRequest, reason: str, detail: Optional[str] = None) -> ChatResponse:
    reason_map = {
...
async def _build_fallback_response(request: ChatRequest, reason: str, note: Optional[str] = None) -> ChatResponse:
...
async def send_message(
    ...
    try:
        ...
        planning_result: PlanningLoopResult = await asyncio.wait_for(
            planner.run(request, history, db),
            timeout=CHAT_RESPONSE_TIMEOUT_SECONDS,
        )
        response = await _build_chat_response(request, planning_result)
        if response.route_updated and not response.route_data and not response.route_id:
            ...
            response = await _build_fallback_response(request, reason="no_route_data", note="Planning returned no usable route data.")
            ...
            response = await _build_fallback_response(request, reason="no_candidates", note="Planning produced no viable candidates.")
    except asyncio.TimeoutError:
        logger.warning("Chat planning timed out; returning fallback route")
        response = await _build_fallback_response(request, reason="timeout")
        ...
        if "credit balance" in message.lower():
            ...
            response = await _build_fallback_response(request, reason="error", note=f"AI request error: {message}")
        ...
        response = await _build_fallback_response(request, reason="error", note=str(e))
    ...
    try:
        ...
        async def run_planning():
            try:
                ...
                planning_result: PlanningLoopResult = await asyncio.wait_for(
                    planner.run(request, history, db, status_callback=status_callback),
                    timeout=CHAT_RESPONSE_TIMEOUT_SECONDS,
                )
...
async def _build_chat_response(request: ChatRequest, planning: PlanningLoopResult) -> ChatResponse:
    ...
    if planning.failure_reason:
        prompt_reference = _build_prompt_reference(request)
        message_lines = [planning.failure_reason]
