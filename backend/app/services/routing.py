...
class RoutingService:
    ...
    VALHALLA_PAVED_SURFACES = {
        "paved", "asphalt", "concrete", "paving_stones", "cobblestone", "sett", "chipseal",
    }
    ...
    def _map_valhalla_edge_surface(self, edge: Dict[str, Any]) -> Tuple[str, str, float]:
        ...
        if surface in self.VALHALLA_PAVED_SURFACES:
            base_surface = "paved"
            confidence = 0.9
    ...
    async def _get_valhalla_surface_data(
        ...
        trace = await self._call_valhalla_trace_attributes(geometry, profile)
    ...
    async def _attach_valhalla_surface(
        ...
        try:
            ...
            route["segmented_surface"] = segmented
        except Exception as exc:
            logger.warning(f"Valhalla trace_attributes failed: {exc}")
        return route
    ...
    async def generate_route(
        ...
        use_valhalla = selected_service == RoutingServiceType.VALHALLA
