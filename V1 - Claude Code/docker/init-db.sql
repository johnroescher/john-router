...
-- Seed minimal location knowledge for UAT areas
INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Boulder, CO', 'mtb', 'trail_system', 'Betasso Preserve',
       'Looped singletrack west of Boulder with hardpack and short punchy climbs.',
       '{"difficulty": "intermediate", "distance_km": 10}'::jsonb, 0.7, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Boulder, CO' AND name = 'Betasso Preserve' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Boulder, CO', 'mtb', 'trail_system', 'Walker Ranch Loop',
       'Classic rocky loop with sustained climbs and technical descents.',
       '{"difficulty": "advanced", "distance_km": 12}'::jsonb, 0.75, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Boulder, CO' AND name = 'Walker Ranch Loop' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Boulder, CO', 'mtb', 'local_tip', 'Mud Season',
       'Front Range trails can be muddy after rain; check open space closure notices.',
       '{"season": "spring"}'::jsonb, 0.6, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Boulder, CO' AND name = 'Mud Season' AND knowledge_type = 'local_tip'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Golden, CO', 'mtb', 'trail_system', 'Apex Park',
       'Steep climb with directional singletrack; popular after-work ride.',
       '{"difficulty": "advanced"}'::jsonb, 0.7, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Golden, CO' AND name = 'Apex Park' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Golden, CO', 'mtb', 'trail_system', 'North Table Mountain',
       'Rolling mesa trails with rocks and short punchy climbs.',
       '{"difficulty": "intermediate"}'::jsonb, 0.65, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Golden, CO' AND name = 'North Table Mountain' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Golden, CO', 'mtb', 'local_tip', 'Directional Trails',
       'Apex has directional days and closures; follow posted signage.',
       '{"note": "directional"}'::jsonb, 0.6, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Golden, CO' AND name = 'Directional Trails' AND knowledge_type = 'local_tip'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Moab, UT', 'mtb', 'trail_system', 'Slickrock Trail',
       'Iconic sandstone domes with steep punchy climbs and grippy rock.',
       '{"difficulty": "advanced"}'::jsonb, 0.8, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Moab, UT' AND name = 'Slickrock Trail' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Moab, UT', 'mtb', 'trail_system', 'Porcupine Rim',
       'Rowdy descent with exposure; shuttle or long loop options.',
       '{"difficulty": "advanced"}'::jsonb, 0.8, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Moab, UT' AND name = 'Porcupine Rim' AND knowledge_type = 'trail_system'
);

INSERT INTO location_knowledge (location_region, sport_type, knowledge_type, name, description, metadata, confidence, source)
SELECT 'Moab, UT', 'mtb', 'local_tip', 'Heat and Water',
       'Best riding is spring or fall; carry extra water and avoid midday heat.',
       '{"season": "spring_fall"}'::jsonb, 0.7, 'seed'
WHERE NOT EXISTS (
    SELECT 1 FROM location_knowledge
    WHERE location_region = 'Moab, UT' AND name = 'Heat and Water' AND knowledge_type = 'local_tip'
);

-- Seed minimal knowledge chunks for RAG
INSERT INTO knowledge_chunks (content, metadata, source, location_region, sport_type)
SELECT
    'Boulder, CO MTB riding includes Betasso Preserve and Walker Ranch with rocky climbs; spring mud season can close trails and summer storms arrive in the afternoon.',
    '{"tags": ["boulder", "mtb", "front_range", "conditions"]}'::jsonb,
    'seed',
    'Boulder, CO',
    'mtb'
WHERE NOT EXISTS (
    SELECT 1 FROM knowledge_chunks
    WHERE location_region = 'Boulder, CO'
      AND source = 'seed'
      AND content LIKE 'Boulder, CO MTB riding includes%'
);

INSERT INTO knowledge_chunks (content, metadata, source, location_region, sport_type)
SELECT
    'Moab, UT riding centers on Slickrock, Porcupine Rim, and Mag 7; bring extra water, avoid wet slickrock, and plan for big temperature swings.',
    '{"tags": ["moab", "mtb", "desert", "conditions"]}'::jsonb,
    'seed',
    'Moab, UT',
    'mtb'
WHERE NOT EXISTS (
    SELECT 1 FROM knowledge_chunks
    WHERE location_region = 'Moab, UT'
      AND source = 'seed'
      AND content LIKE 'Moab, UT riding centers%'
);

INSERT INTO knowledge_chunks (content, metadata, source, location_region, sport_type)
SELECT
    'Golden, CO trails like Apex Park and North Table Mountain are steep and directional; weekends are busy, so start early for smoother riding.',
    '{"tags": ["golden", "mtb", "traffic", "directional"]}'::jsonb,
    'seed',
    'Golden, CO',
    'mtb'
WHERE NOT EXISTS (
    SELECT 1 FROM knowledge_chunks
    WHERE location_region = 'Golden, CO'
      AND source = 'seed'
      AND content LIKE 'Golden, CO trails like Apex Park%'
);

-- Planning sessions (Ride Brief Loop persistence)
CREATE TABLE IF NOT EXISTS planning_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    conversation_id UUID REFERENCES chat_conversations(id) ON DELETE SET NULL,
    user_id UUID REFERENCES users(id) ON DELETE SET NULL,
    intent_object JSONB DEFAULT '{}'::jsonb NOT NULL,
    ride_brief JSONB DEFAULT '{}'::jsonb NOT NULL,
    discovery_plan JSONB DEFAULT '{}'::jsonb NOT NULL,
    ingredient_set JSONB DEFAULT '{}'::jsonb NOT NULL,
    critique_report JSONB DEFAULT '{}'::jsonb NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_planning_candidates_session_id ON planning_candidates (session_id);
