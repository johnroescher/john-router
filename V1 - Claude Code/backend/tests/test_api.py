...
class TestRoutingEndpoints:
    ...
    @patch("app.api.routes.get_route_metadata_service", new_callable=AsyncMock)
    ...
    def test_generate_route_success(self, mock_routing, mock_analysis, mock_validation, mock_metadata, client):
        ...
        mock_metadata.return_value = AsyncMock(build_segment_metadata=AsyncMock(return_value=[]))
    ...
    @patch("app.api.routes.get_analysis_service", new_callable=AsyncMock)
    @patch("app.api.routes.get_routing_service", new_callable=AsyncMock)
    def test_generate_loop_route(self, mock_routing, mock_analysis, mock_validation, mock_metadata, client):
...
class TestChatEndpoints:
    ...
    def test_send_message(self, mock_get_planner, mock_build_response, client):
        ...
        response = client.post(
            ...
            json={
                ...
                "map_center": {"lat": 40.015, "lng": -105.2705},
            },
        )
    ...
    def test_chat_requires_start_location(self, client):
        """Test chat requests without location prompt for clarification."""
        response = client.post(
            "/api/chat/message",
            json={
                "message": "Plan a loop for tomorrow",
                "conversation_id": None,
            },
        )
        ...
        assert "Where should the route start?" in (data.get("clarification_question") or "")
