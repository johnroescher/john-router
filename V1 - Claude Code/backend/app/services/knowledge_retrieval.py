...
class KnowledgeRetrievalService:
    ...
    async def retrieve_knowledge(
        ...
        # Keyword fallback when vector search is unavailable or insufficient
        if len(chunks) < limit:
            try:
        ...
        ranked_chunks = self._rank_chunks(self._dedupe_chunks(chunks), query)
    ...
    async def _keyword_search(
        ...
        conditions = []
        if terms:
            term_conditions = [KnowledgeChunk.content.ilike(f"%{term}%") for term in terms]
            conditions.append(or_(*term_conditions))
        else:
            conditions.append(KnowledgeChunk.content.ilike(f"%{query}%"))

        if location_region:
            conditions.append(KnowledgeChunk.location_region == location_region)
        if sport_type:
            conditions.append(KnowledgeChunk.sport_type == sport_type)

        result = await db.execute(
            select(KnowledgeChunk).where(and_(*conditions)).limit(max(limit * 3, limit))
        )
        rows = result.scalars().all()

        def score_for(content: str) -> float:
            ...
            if terms:
                ...
                base += 0.4 * (matches / max(len(terms), 1))
            return min(base, 1.0)

        chunks: List[KnowledgeChunkSchema] = []
        for row in rows:
            score = score_for(row.content or "")
            if location_region and row.location_region == location_region:
                score = min(1.0, score + 0.1)
            if sport_type and row.sport_type == sport_type:
                score = min(1.0, score + 0.05)
            chunks.append(KnowledgeChunkSchema(
                id=row.id,
                content=row.content,
        ...
        chunks.sort(key=lambda c: c.relevance_score or 0.0, reverse=True)
        return chunks[:limit]
    ...
    def _dedupe_chunks(self, chunks: List[KnowledgeChunkSchema]) -> List[KnowledgeChunkSchema]:
        seen = set()
        deduped = []
        for chunk in chunks:
            key = str(chunk.id) if chunk.id else chunk.content[:80]
            if key in seen:
                continue
            seen.add(key)
